...
def bearing_signal_model_local(d, D, contact_angle, n, fault_type, fr, fc, fd, fm, N, variance_factor, fs, k, zita, fn, Lsdof, SNR_dB, qAmpMod=1):
    ...
    if fault_type == 'inner':
        ...
        L_resampled = 1024  # 重新采样后的长度
        t_resampled = resample(t, L_resampled)

        cos_fm_t = np.cos(2 * np.pi * fm * t_resampled)
        cumsum_cos_fm_t = np.cumsum(cos_fm_t / fs)
        q_resampled = 1 + qAmpMod * np.cos(2 * np.pi * fc * t_resampled + 2 * np.pi * fd * cumsum_cos_fm_t)

# 为了与原始长度相匹配，我们需要将q_resampled扩展回原始长度
        q = np.interp(t, t_resampled, q_resampled)
...
def main():
    ...
    contactAngle = 9 * np.pi / 180  # contact angle
    faultType = 'inner'

    # Speed profile
    N = 2048  # number of points per revolution
    Ltheta = 10000 * N  # signal length
    theta = np.linspace(0, (Ltheta-1) * 2 * np.pi / N, Ltheta)
    fc = 10
    fd = 0.08 * fc
    fm = 0.1 * fc
    ...
    tLocal, xLocal, xNoiseLocal, frTimeLocal, meanDeltaTLocal, varDeltaTLocal, meanDeltaTimpOverLocal, varDeltaTimpOverLocal, errorDeltaTimpLocal = bearing_signal_model_local(d, D, contactAngle, n, faultType, fr, fc, fd, fm, N, varianceFactor, fs, k, zita, fn, Lsdof, SNR_dB, qAmpMod)

    # Plot original signal
    plt.figure()
    plt.plot(tLocal, xLocal)
    plt.title('原始信号')
    plt.xlabel('时间 (s)')
    plt.ylabel('幅度')
